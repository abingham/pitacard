...
class HtmlHandler(xml.sax.handler.ContentHandler):
    ...
    def _begin_span(self, style, tag=None):
        ...
        try:
            cluck = [item in style.split(';')]
        except:
            cluck = [re.sub(';', '', style)]
        for attr, val in [item.split(':', 1) for item in cluck]:
...
#H<sub>2</sub>O H<sup>+</sup>
        ...
        if name in ['a', 'u', 'i', 'b', 'sub', 'sup']:
            ...
            if name == 'sup':
                tag = self.textbuf.create_tag()
		tag.set_property('scale', pango.SCALE_SMALL)
                tag.set_property('rise', int('5500'))
            if name == 'sub':
                tag = self.textbuf.create_tag()
		tag.set_property('scale', pango.SCALE_SMALL)
                tag.set_property('rise', int('-2000'))
            tag.is_anchor = True
...
#H<sub>2</sub>O H<sup>+</sup>
    ...
    def endElement(self, name):
        ...
        if name == 'p':
            ...
            self._insert_text("\n")
        elif name in ['img', 'body', 'a', 'u', 'i', 'b', 'sub', 'sup']:
            pass
        else:
            warnings.warn("Unhandled element '%s'" % name)
        self._end_span()
...
class HtmlTextView(gtk.TextView):
    ...
    def display_html_replace(self, html):
        buffer = self.get_buffer()
        buffer.set_text('')
        eob = buffer.get_end_iter()
        ## this works too if libxml2 is not available
